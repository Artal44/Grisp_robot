...
controller({Dt, Angle, Speed}, {Adv_V_Goal, Adv_V_Ref}, {Turn_V_Goal, Turn_V_Ref}, N) ->
    ...
    if N rem 10 == 0 ->
        io:format("[LOG] SPEED_CTRL | Time=~p | Setpoint=~p | Input=~p | Output=~p~n", [Timestamp, Adv_V_Ref_New, Speed, Target_angle]);
       true ->
        ok
    end,
    ...
    if N rem 10 == 0 ->
        io:format("[LOG] STABILITY_CTRL | Time=~p | Setpoint=~p | Input=~p | Output=~p~n", [Timestamp, Target_angle, Angle, Acc]);
       true ->
        ok
    end,
